...
public interface UserRepository extends JpaRepository<User, Long> {
	...
	List<User> findByManagerIdAndIsApprovedAndIsVerified(Long managerId, Boolean isApproved, Boolean isVerified);
	...
	@Query(value = "WITH RECURSIVE subordinate AS (SELECT  id, created_at, updated_at, email, full_name, is_active, is_approved, is_verified, password, "+
			"user_name, grade_id, manager_id, 0 AS level FROM user WHERE manager_id=:managerId "+
			"UNION ALL "+
			"SELECT e.id, e.created_at, e.updated_at, e.email, e.full_name, e.is_active, e.is_approved, e.is_verified, e.password, "+
			"e.user_name, e.grade_id, e.manager_id, level + 1 FROM user e JOIN subordinate s ON e.manager_id = s.id) "+
			"SELECT s.id, s.created_at, s.updated_at, s.email, s.full_name, s.is_active, s.is_approved, s.is_verified, s.password, s.user_name, s.grade_id, s.manager_id "+
